...
router.get("/", async (req, res) => {
  let departmentAdmins = req.body.filter
    ? await DepartmentAdmin.find(req.body.filter)
    : await DepartmentAdmin.find({});
...
router.put("/:departmentAdminId", async (req, res) => {
  try {
    ...
    let departmentAdmin = await DepartmentAdmin.findById(
...
router.delete("/:departmentAdminId", async (req, res) => {
  try {
    ...
    await DepartmentAdmin.deleteOne({ _id: req.params.departmentAdminId });
    res.json({
      success: true,
